__require = function e(t, n, r) {
  ...
  bulletManager: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "dcd96LwpllLEqS87xsqorES", "bulletManager");
    "use strict";
    cc.Class({
      ...
      addBullet: function addBullet(data) {
        var bullet = this.redBulletPool.get();
        bullet || (bullet = cc.instantiate(this.redBulletPrefab));
      ...
      setBulletData: function setBulletData() {
        this.redNumber = this.redNumber > 35 ? 0 : this.redNumber;
        var bulletDir = Math.floor(4 * Math.random());
        var x = 0;
        var y = 0;
        switch (bulletDir) {
         case 0:
          x = -GLB.limitX + 50 * Math.random();
         ...
         case 2:
          x = -GLB.limitX + Math.random() * (2 * GLB.limitX);
          y = GLB.limitYTop - 50 * Math.random();
          break;

         case 3:
          x = -GLB.limitX + Math.random() * (2 * GLB.limitX);
          y = -GLB.limitYBottom + 50 * Math.random();
        }
        var bulletPos = cc.p(x, y);
        var angle = 6.28 * Math.random();
        var speed = 0;
        speed = this.redNumber > 30 ? 200 : 100;
        this.redNumber++;
        var data = {
          bulletPos: bulletPos,
          angle: angle,
      ...
      recycleBullet: function recycleBullet(target) {
        switch (target.bulletType) {
         case "red":
          this.redBulletPool.put(target);
          break;

         case "blue":
          this.buleBulletPool.put(target);
          break;

         case "green":
          this.greenBulletPool.put(target);
          break;

         case "radintion":
          this.radintionBulletPool.put(target);
        }
      },
      diffuseBullet: function diffuseBullet(bulletType, position) {
        var angle = .628;
        for (var i = 1; i < 11; i++) {
          if ("blue" === bulletType) {
      ...
      radintionBullet: function radintionBullet(obj, position) {
        var bullet = this.radintionBulletPool.get();
        bullet || (bullet = cc.instantiate(this.radintionBulletPrefab));
        bullet.parent = Game.BulletManager.node;
  ...
  bullet: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "a5b25JG7SlJrYwVGG3gvsWZ", "bullet");
    "use strict";
    cc.Class({
      extends: cc.Component,
      init: function init(speed, angle) {
        this.speed = speed;
        this.angle = angle;
      },
      ...
      update: function update(dt) {
        ...
        (this.node.x < -GLB.limitX - 20 || this.node.x > GLB.limitX + 20 || this.node.y < -GLB.limitYBottom || this.node.y > GLB.limitYTop) && this.destroyBullet();
      }
    });
    cc._RF.pop();
  }, {} ],
  clickManager: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      touchmove: function touchmove(event) {
        ...
        x = x > GLB.limitX - GLB.range ? GLB.limitX - GLB.range : x < -GLB.limitX + GLB.range ? -GLB.limitX + GLB.range : x;
        y = y > GLB.limitYTop - GLB.range ? GLB.limitYTop - GLB.range : y < -GLB.limitYBottom + GLB.range ? -GLB.limitYBottom + GLB.range : y;
        this.playerPos = cc.p(x, y);
      },
      update: function update(dt) {
        this.bTouch && this.node.dispatchEvent(new cc.Event.EventCustom(clientEvent.eventType.playerMove, true));
      }
    });
    cc._RF.pop();
  }, {} ],
  ...
  gameManager: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      networkStateNotify: function networkStateNotify(netNotify) {
        ...
        netNotify.userID !== GLB.userInfo.id && (GLB.isRoomOwner = true);
        console.log("\u73a9\u5bb6\uff1a" + netNotify.userID + " state:" + netNotify.state);
        netNotify.userID !== GLB.userInfo.id && (this.isRivalLeave = true);
      ...
      frameUpdate: function frameUpdate(rsp) {
        for (var i = 0; i < rsp.frameItems.length; i++) {
          ...
          var cpProto = JSON.parse(info.cpProto);
          info.cpProto.indexOf(GLB.POSITION) >= 0 && (GLB.userInfo.id === info.srcUserID ? Game.PlayerManager.self.setDirect(cpProto.position) : Game.PlayerManager.rival.setDirect(cpProto.position));
          info.cpProto.indexOf(GLB.ADD_BULLET) >= 0 && Game.BulletManager.addBullet(cpProto.bulletData);
          info.cpProto.indexOf(GLB.HURT) >= 0 && (GLB.userInfo.id === cpProto.playerId ? Game.PlayerManager.self.hurt() : Game.PlayerManager.rival.hurt());
          info.cpProto.indexOf(GLB.SHOOT_GUN_ITEM) >= 0 && Game.ItemManager.addItem(cpProto.itemData);
          if (info.cpProto.indexOf(GLB.DIFFUSE_ITEM_GET) >= 0) if (GLB.userInfo.id === cpProto.playerId) {
            var position = Game.PlayerManager.self.node.getPosition();
            Game.BulletManager.diffuseBullet("blue", position);
          } else {
            var _position = Game.PlayerManager.rival.node.getPosition();
            Game.BulletManager.diffuseBullet("green", _position);
          }
          if (info.cpProto.indexOf(GLB.RADINTION_ITEM_GET) >= 0) if (GLB.userInfo.id === cpProto.playerId) {
            var obj = Game.PlayerManager.rival.node;
            var _position2 = Game.PlayerManager.self.node.getPosition();
            Game.BulletManager.radintionBullet(obj, _position2);
          } else {
            var _obj = Game.PlayerManager.self.node;
            var _position3 = Game.PlayerManager.rival.node.getPosition();
            Game.BulletManager.radintionBullet(_obj, _position3);
          }
        }
      },
      ...
      loginServer: function loginServer() {
        this.network.isConnected() ? this.network.send("connector.rankHandler.updateScore", {
  ...
  itemManager: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "65924VFLPVOr7memIp0v8iY", "itemManager");
    "use strict";
    var mvs = require("Matchvs");
    cc.Class({
      extends: cc.Component,
      ...
      scheduleItemSpawn: function scheduleItemSpawn() {
        clearInterval(this.scheduleItemId);
        this.scheduleItemId = setInterval(function() {
          if (Game.GameManager.gameState === GameState.Over || !GLB.isRoomOwner) return;
      ...
      setBulletData: function setBulletData() {
        var itemType = Math.floor(2 * Math.random());
      ...
      recycleItem: function recycleItem(target) {
        switch (target.itemType) {
         case "diffuse":
          this.diffuseItemPool.put(target);
          break;

         case "radintion":
          this.radintionItemPool.put(target);
        }
        this.itemsNumber--;
      },
  ...
  item: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "57c08Z2SglJyaQpWFEm3npb", "item");
    "use strict";
    cc.Class({
      extends: cc.Component,
      properties: {},
  ...
  "matchvs.all": [ function(require, module, exports) {
    (function(global) {
      ...
      function commEngineStateCheck(engineState, roomLoock, type) {
        if ((engineState & ENGE_STATE.HAVE_INIT) !== ENGE_STATE.HAVE_INIT) return -2;
        if ((engineState & ENGE_STATE.INITING) === ENGE_STATE.INITING) return -3;
        if ((engineState & ENGE_STATE.HAVE_LOGIN) !== ENGE_STATE.HAVE_LOGIN) return -4;
        if ((engineState & ENGE_STATE.LOGINING) === ENGE_STATE.LOGINING) return -5;
        if ((engineState & ENGE_STATE.CREATEROOM) === ENGE_STATE.CREATEROOM) return -7;
        if ((engineState & ENGE_STATE.JOIN_ROOMING) === ENGE_STATE.JOIN_ROOMING) return -7;
        if ((engineState & ENGE_STATE.LOGOUTING) === ENGE_STATE.LOGOUTING) return -11;
        if (1 === type) {
          if ((engineState & ENGE_STATE.IN_ROOM) !== ENGE_STATE.IN_ROOM) return -6;
          if ((engineState & ENGE_STATE.LEAVE_ROOMING) === ENGE_STATE.LEAVE_ROOMING) return -10;
        } else if (2 === type) {
          if ((engineState & ENGE_STATE.IN_ROOM) === ENGE_STATE.IN_ROOM) return -8;
          if ((engineState & ENGE_STATE.LEAVE_ROOMING) === ENGE_STATE.LEAVE_ROOMING) return -10;
        } else if (3 === type && (engineState & ENGE_STATE.LEAVE_ROOMING) === ENGE_STATE.LEAVE_ROOMING) return -10;
        return 0;
      }
      (function e(t, n, r) {
        function s(o, u) {
        ...
        if ("undefined" !== typeof wx) {
          ...
          MatchvsNetWork = function MatchvsNetWork(host, callback) {
            ...
            this.send = function(message) {
              ...
              socket.readyState === WebSocket.OPEN ? socket.send(message) : bufQueue.push(message);
            };
            this.close = function() {
              socket && socket.close();
            };
              ...
              socket.onmessage = function(event) {
                ...
                reader.onload = function(evt) {
                  if (evt.target.readyState === FileReader.DONE) {
                    ...
                    mCallBack.onMsg(dataView);
                  } else mCallBack.onErr(1606, "[err]parse fail");
                };
              };
              socket.onopen = function(event) {
                MatchvsLog.logI("Create the socket is success :" + mHost + " socket is " + socket.hashcode);
                while (bufQueue.length > 0) socket.send(bufQueue.pop());
                mCallBack.onConnect && mCallBack.onConnect(mHost);
              };
              socket.onclose = function(e) {
                mCallBack.onDisConnect && mCallBack.onDisConnect(mHost, e);
        ...
        var NetWorkCallBackImp = function NetWorkCallBackImp(engine) {
          ...
          this.onMsg = function(dataView) {
            ...
            switch (packet.header.cmd) {
             case MATCHVS_USER_LOGIN_RSP:
              if (200 === packet.payload.getStatus()) engine.mEngineState |= ENGE_STATE.HAVE_LOGIN; else {
              ...
              } else engine.mRsp.loginResponse(new MsLoginRsp(packet.payload.getStatus(), engine.mRecntRoomID));
              break;

             case MATCHVS_ROOM_JOIN_RSP:
              if (200 === packet.payload.getStatus()) {
                ...
                null == engine.mHotelHeartBeatTimer && (engine.mHotelHeartBeatTimer = setInterval(engine.hotelHeartBeat, HEART_BEAT_INTERVAL));
              } else {
             ...
             case MATCHVS_ROOM_CHECK_IN_RSP:
              200 !== packet.payload.getStatus() && engine.mRsp.errorResponse && engine.mRsp.errorResponse(packet.payload.getStatus(), "Server Response Error");
             ...
             case MATCHVS_HOTEL_NOTIFY:
              ...
              0 === srcUserID ? engine.mRsp.gameServerNotify && engine.mRsp.gameServerNotify(new MsGameServerNotifyInfo(packet.payload.getSrcuid(), utf8ByteArrayToString(packet.payload.getCpproto()))) : engine.mRsp.sendEventNotify && engine.mRsp.sendEventNotify(new MsSendEventNotify(packet.payload.getSrcuid(), utf8ByteArrayToString(packet.payload.getCpproto())));
              break;
             ...
             case CMD_KICK_PLAYER_NOTIFY:
              if (packet.payload.getUserid().toString() === "" + engine.mUserID && null != engine.mHotelHeartBeatTimer) {
             ...
             case CMD_FRAME_SYNCNOTIFY_CMDID:
              var frameData = [];
              while (frameCache.length > 0) frameData.push(frameCache.pop());
             ...
             case CMD_SET_ROOM_PROPERTY_RSP:
              200 !== packet.payload.getStatus() && engine.errorResponse && engine.errorResponse(packet.payload.getStatus(), "Server response error");
          ...
          this.onDisConnect = function(host, event) {
            ...
            if (host.endsWith(HttpConf.HOST_GATWAY_ADDR)) {
              (engine.mEngineState & ENGE_STATE.LOGOUTING) !== ENGE_STATE.LOGOUTING && (event && event.code && (1e3 === event.code || 1005 === event.code) ? MatchvsLog.logI("gateway close is friend") : engine.mRsp.errorResponse && engine.mRsp.errorResponse(1001, "gateway network error"));
              ...
              (engine.mEngineState & ENGE_STATE.LEAVE_ROOMING) !== ENGE_STATE.LEAVE_ROOMING && (event && event.code && (1e3 === event.code || 1005 === event.code) ? MatchvsLog.logI("hotel close is friend") : engine.mRsp.errorResponse && engine.mRsp.errorResponse(1001, "hotel network error"));
              if (null != engine.mHotelHeartBeatTimer) {
                clearInterval(engine.mHotelHeartBeatTimer);
                engine.mHotelHeartBeatTimer = null;
              }
              engine.mEngineState &= ~ENGE_STATE.LEAVE_ROOMING;
      ...
      MatchvsEngine.prototype.registerUser = function() {
        ...
        rep.onMsg = function(buf) {
          ...
          } else this.rsp(new MsRegistRsp(obj.status, 0, "0", buf, ""));
        };
  ...
  player: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      onCollisionEnter: function onCollisionEnter(other) {
        ...
        "redBullet" === group ? this.sendHurtMsg() : "greenBullet" === group && "self" === this.node.name ? this.sendHurtMsg() : "blueBullet" === group && "rival" === this.node.name && this.sendHurtMsg();
        if ("diffuseItem" === group) {
          this.sendDiffuseItemMsg(other.id);
          other.getComponent("item").destroyItem();
          this.unmatchedState("twinkle");
        }
        if ("radiationItem" === group) {
      ...
      hurt: function hurt() {
        if (this.bUnmatched) return;
        if (Game.GameManager.playerDie) return;
        cc.audioEngine.play(this.boomClip, false, 1);
        if (this.heart > 0) {
          this.heart--;
          if (1 === this.heart) this.unmatchedState("damage"); else if (this.heart <= 0) {
      ...
      unmatchedState: function unmatchedState(animation) {
        var _this = this;
        this.bUnmatched = true;
        this.shield.active = true;
        this.node.getComponent(cc.Animation).play(animation);
        this.scheduleOnce(function() {
          _this.bUnmatched = false;
          _this.shield.active = false;
          _this.node.getComponent(cc.Animation).pause(animation);
          _this.node.opacity = 255;
        }, 3);
      },
      setPlayerPosBk: function setPlayerPosBk() {
        this.playerPosBk = this.node.getPosition();
      },
      setDirect: function setDirect(position) {
        this.node.setPosition(position);
      },
      sendHurtMsg: function sendHurtMsg() {
        Game.GameManager.gameState === GameState.Play && GLB.isRoomOwner && mvs.engine.sendFrameEvent(JSON.stringify({
          action: GLB.HURT,
          playerId: this.playerId
        }));
      },
      sendDiffuseItemMsg: function sendDiffuseItemMsg(itemId) {
        Game.GameManager.gameState === GameState.Play && GLB.isRoomOwner && mvs.engine.sendFrameEvent(JSON.stringify({
          action: GLB.DIFFUSE_ITEM_GET,
          playerId: this.playerId
        }));
      },
      sendRadintionItemMsg: function sendRadintionItemMsg(itemId) {
        Game.GameManager.gameState === GameState.Play && GLB.isRoomOwner && mvs.engine.sendFrameEvent(JSON.stringify({
  ...
  radintionBullet: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "bc59aA6o+5J0bc4DHmyD+2u", "radintionBullet");
    "use strict";
    cc.Class({
      extends: cc.Component,
      ...
      twinkleState: function twinkleState() {
        var _this = this;
        this.node.getComponent(cc.Animation).play("twinkle");
        this.scheduleOnce(function() {
          _this.node.getComponent(cc.Animation).stop("twinkle");
          _this.node.opacity = 255;
          _this.destroyBullet();
        }, 3);
      },
      update: function update(dt) {
        ...
        (this.node.x < -GLB.limitX || this.node.x > GLB.limitX || this.node.y < -GLB.limitYBottom || this.node.y > GLB.limitYTop) && this.destroyBullet();
      }
    });
    cc._RF.pop();
  }, {} ],
  rankUserInfo: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      setData: function setData(data) {
        this.rankCntLb && (this.rankCntLb.string = data.rank);
        this.userNameLb.string = data.userName;
        data.headIcon && "-" !== data.headIcon && cc.loader.load({
          url: data.headIcon,
          type: "png"
        }, function(err, texture) {
          var spriteFrame = new cc.SpriteFrame(texture, cc.Rect(0, 0, texture.width, texture.height));
  ...
  uiExit: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      sure: function sure() {
        if (!Game.GameManager.exitGame) return;
  ...
  uiGamePanel: [ function(require, module, exports) {
    ...
    cc.Class((_cc$Class = {
      ...
      sendDirectMsg: function sendDirectMsg(position) {
        Game.GameManager.gameState === GameState.Play && mvs.engine.sendFrameEvent(JSON.stringify({
          action: GLB.POSITION,
          position: position
        }));
      },
      sendAddBulletMsg: function sendAddBulletMsg() {
  ...
  uiLobbyPanel: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      start: function start() {
        ...
        Game.GameManager.nickName ? this.nodeDict["name"].getComponent(cc.Label).string = Game.GameManager.nickName : this.nodeDict["name"].getComponent(cc.Label).string = GLB.userInfo.id;
        Game.GameManager.avatarUrl && cc.loader.load({
      ...
      rank: function rank() {
        Game.GameManager.blockInput();
        Game.GameManager.network.isConnected() ? Game.GameManager.network.send("connector.rankHandler.getRankData", {
          account: GLB.userInfo.id + "",
          game: GLB.GAME_NAME
        }) : Game.GameManager.network.connect(GLB.IP, GLB.PORT, function() {
          Game.GameManager.network.send("connector.entryHandler.login", {
      ...
      inviteFriend: function inviteFriend() {
        window.wx && wx.shareAppMessage({
          imageUrl: "https://data.tianziyou.com/matchvsGamesRes/logo/extremeEvasionLogo.png"
        });
      }
    });
    cc._RF.pop();
  ...
  uiMatching1v1: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "3a859QUfedBCLYVA5NV2KCp", "uiMatching1v1");
    "use strict";
    var uiPanel = require("uiPanel");
    var mvs = require("Matchvs");
    cc.Class({
  ...
  uiMatching2v2Ver: [ function(require, module, exports) {
    "use strict";
  ...
  uiMatching2v2: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "d71c6Mv4L1LbILkqJcx5C7M", "uiMatching2v2");
    "use strict";
    var uiPanel = require("uiPanel");
    var mvs = require("Matchvs");
    cc.Class({
  ...
  uiMatching: [ function(require, module, exports) {
    "use strict";
  ...
  uiVsResult: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      start: function start() {
        var isLose = !Game.GameManager.isRivalLeave && Game.PlayerManager.self.heart < Game.PlayerManager.rival.heart;
        ...
        if (isLose) cc.audioEngine.play(this.loseClip, false, 1); else {
...
}, {}, [ "DataFunc", "Globals", "Matchvs", "MatchvsEngine", "UIFunc", "clientEvent", "eventListenerSelf", "msgType", "network", "pomeloBuild", "uiPanel", "playerIcon", "rankUserInfo", "resultPlayerIcon", "roomInfo", "roomUserInfo", "uiCreateRoom", "uiExit", "uiLobbyPanel", "uiLogin", "uiMaskLayout", "uiMatching", "uiMatching1v1", "uiMatching1v1Ver", "uiMatching2v2", "uiMatching2v2Ver", "uiRankPanel", "uiResult", "uiResultPanel", "uiRoom", "uiRoomList", "uiTip", "uiVsResult", "uiWaitingForServer", "bullet", "item", "bulletManager", "clickManager", "gameManager", "itemManager", "playerManager", "player", "radintionBullet", "uiGamePanel", "matchvs.all" ]);
